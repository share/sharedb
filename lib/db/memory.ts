...
// Callback should be called as callback(error, [list of ops]);
    public getOps(collection, id, from, to, options, callback) {
        ...
        process.nextTick(function () {
            ...
            var ops = MemoryDB.clone(opLog.slice(from, to));
            if (!includeMetadata) {
...
// regardless of query by default
    public query(collection, query, fields, options, callback) {
        ...
        process.nextTick(function () {
            ...
            try {
                var result: any = db._querySync(snapshots, query, options);
...
// - extra: (optional) other types of results, such as counts
    public _querySync(snapshots, query, options) {
        return {snapshots: snapshots};
    };

    public _writeOpSync(collection, id, op) {
...
// it should be considered a delete
    public _writeSnapshotSync(collection, id, snapshot) {
        var collectionDocs = this.docs[collection] || (this.docs[collection] = {});
        if (!snapshot.type) {
            delete collectionDocs[id];
        } else {
            collectionDocs[id] = MemoryDB.clone(snapshot);
        }
    };

    public _getSnapshotSync(collection, id, includeMetadata) {
        ...
        var snapshot;
        if (doc) {
            var data = MemoryDB.clone(doc.data);
            var meta = (includeMetadata) ? MemoryDB.clone(doc.m) : undefined;
...
class MemorySnapshot {
    ...
    constructor(public id, version, public type, public data?, meta?) {
        this.id = id;
        this.v = version;
        this.type = type;
        this.data = data;
