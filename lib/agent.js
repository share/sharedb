...
Agent.prototype._reply = function(request, err, message) {
  if (err) {
    if (typeof err === 'string') {
      request.error = {code: 4001, message: err};
    } else {
      if (err.stack) {
...
Agent.prototype._open = function() {
  ...
  this.stream.on('data', function(chunk) {
    ...
    // FIXME: `request` is accessed out of scope. Should this code block actually be inside the try?
    agent.backend.trigger('receive', agent, request, function(err) { // jshint ignore:line
