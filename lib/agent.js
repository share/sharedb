...
Agent.prototype._subscribeToStream = function(collection, id, stream) {
  ...
  stream.on('data', function(data) {
    ...
    if (agent.presence.processPresenceData(data)) return;
  ...
  stream.on('end', function() {
    var presence = agent.presence.createPresence(collection, id);
...
Agent.prototype._handleMessage = function(request, callback) {
  try {
    ...
    switch (request.a) {
      ...
      case 'p':
        return this._handlePresenceMessage(request, callback);
      default:
...
Agent.prototype._handlePresenceMessage = function(request, callback) {
  var presence = this.presence.createPresence(request.c, request.d, request.p, request.v, request.r, request.seq);
  if (presence.seq <= this.presence.maxPresenceSeq) {
    return process.nextTick(function() {
      callback(new ShareDBError(4026, 'Presence data superseded'));
    });
  }
  this.presence.maxPresenceSeq = presence.seq;
