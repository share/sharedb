...
Agent.prototype._subscribeToStream = function(collection, id, stream) {
  ...
  stream.on('data', function(op) {
    if (op.error) {
      ...
      console.error('Doc subscription stream error', collection, id, op.error);
      return;
    }
    if (agent.clientId === op.src && collection === (op.i || op.c)) {
      // This was the client that submitted the op, so no need to send it
      // back. It will be acknowledged in the submit reply. Check the
      // collection as well, since we need to send the op to other projections
      return;
    }
...
Agent.prototype._subscribeToQuery = function(emitter, queryId, collection, query) {
  ...
  emitter.onOp = function(op) {
    var id = op.d;
    // Don't send the op if we are already subscribed to the same doc directly
    var docs = agent.subscribedDocs[collection];
    if (docs && docs[id]) return;
    agent._sendOp(collection, id, op);
  };
};
