...
Agent.prototype._open = function() {
  ...
  this.stream.on('data', function(chunk) {
    ...
    if (typeof chunk !== 'object') {
      var err = {code: ERROR_CODE.BADLY_FORMED_MESSAGE, message: 'Received non-object message'};
...
Agent.prototype._handleMessage = function(request, callback) {
  try {
    ...
    if (errMessage) return callback({code: ERROR_CODE.BADLY_FORMED_MESSAGE, message: errMessage});
    ...
    switch (request.a) {
      ...
      case 'op':
        ...
        if (!op) return callback({code: ERROR_CODE.BADLY_FORMED_MESSAGE, message: 'Invalid op message'});
...
Agent.prototype._submit = function(collection, id, op, callback) {
  ...
  this.backend.submit(this, collection, id, op, null, function(err, ops) {
    ...
    if (err) {
      ...
      if (err.code === ERROR_CODE.OP_ALREADY_SUBMITTED) return callback(null, ack);
      return callback(err);
    }
