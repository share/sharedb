...
Agent.prototype._sendPresence = function(request, callback) {
  ...
  const from = version
  this.backend.db.getOpsToSnapshot(collection, id, from, snapshot, null, function(err, ops) {
    if (err) return callback(err);
    ...
    backend.trigger('transformPresence', this.agent, request, function(err) {
      if (err) return callback(err);
      ...
      this.backend.sendPresence(message, function(err) {
        if (err) return callback(err);
      });
    })
  })
