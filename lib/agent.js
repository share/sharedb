...
var Agent = /** @class */ (function () {
  function Agent(backend, stream) {
    ...
    this.send(this._initMessage(message_actions_1.ACTIONS.initLegacy));
  }
  ...
  Agent.prototype._subscribeToQuery = function (emitter, queryId, collection, query) {
    ...
    emitter.onExtra = function (extra) {
      agent.send({ a: message_actions_1.ACTIONS.queryUpdate, id: queryId, extra: extra });
    };
    emitter.onDiff = function (diff) {
      ...
      agent.send({ a: message_actions_1.ACTIONS.queryUpdate, id: queryId, diff: diff });
    };
  ...
  Agent.prototype._sendOp = function (collection, id, op) {
    var message = {
      a: message_actions_1.ACTIONS.op,
      c: collection,
      d: id,
      v: op.v,
      src: op.src,
  ...
  Agent.prototype._checkRequest = function (request) {
    if (
      request.a === message_actions_1.ACTIONS.queryFetch ||
      request.a === message_actions_1.ACTIONS.querySubscribe ||
      request.a === message_actions_1.ACTIONS.queryUnsubscribe
    ) {
      // Query messages need an ID property.
      if (typeof request.id !== 'number') return 'Missing query ID';
    } else if (
      request.a === message_actions_1.ACTIONS.op ||
      request.a === message_actions_1.ACTIONS.fetch ||
      request.a === message_actions_1.ACTIONS.subscribe ||
      request.a === message_actions_1.ACTIONS.unsubscribe ||
      request.a === message_actions_1.ACTIONS.presence
    ) {
      // Doc-based request.
      if (request.c != null) {
      ...
      if (request.a === message_actions_1.ACTIONS.presence) {
        if (typeof request.id !== 'string' || util.isDangerousProperty(request.id)) {
          return 'Invalid presence ID';
        }
      }
    } else if (
      request.a === message_actions_1.ACTIONS.bulkFetch ||
      request.a === message_actions_1.ACTIONS.bulkSubscribe ||
      request.a === message_actions_1.ACTIONS.bulkUnsubscribe
    ) {
      // Bulk request
      if (request.c != null) {
        if (typeof request.c !== 'string' || util.isDangerousProperty(request.c)) {
  ...
  Agent.prototype._handleMessage = function (request, callback) {
    try {
      ...
      if (errMessage)
        return callback(new ShareDBError(ERROR_CODE.ERR_MESSAGE_BADLY_FORMED, errMessage));
      this._checkFirstMessage(request);
      switch (request.a) {
        case message_actions_1.ACTIONS.handshake:
          if (request.id) this.src = request.id;
          this._setProtocol(request);
          return callback(null, this._initMessage(message_actions_1.ACTIONS.handshake));
        case message_actions_1.ACTIONS.queryFetch:
        ...
        case message_actions_1.ACTIONS.querySubscribe:
        ...
        case message_actions_1.ACTIONS.op:
          ...
          if (!op)
            return callback(
              new ShareDBError(ERROR_CODE.ERR_MESSAGE_BADLY_FORMED, 'Invalid op message'),
            );
          return this._submit(request.c, request.d, op, callback);
        case message_actions_1.ACTIONS.snapshotFetch:
          return this._fetchSnapshot(request.c, request.d, request.v, callback);
        case message_actions_1.ACTIONS.snapshotFetchByTimestamp:
          return this._fetchSnapshotByTimestamp(request.c, request.d, request.ts, callback);
        case message_actions_1.ACTIONS.presence:
        ...
        case message_actions_1.ACTIONS.presenceSubscribe:
          if (!this.backend.presenceEnabled) return;
          return this._subscribePresence(request.ch, request.seq, callback);
        case message_actions_1.ACTIONS.presenceUnsubscribe:
          return this._unsubscribePresence(request.ch, request.seq, callback);
        case message_actions_1.ACTIONS.presenceRequest:
          return this._requestPresence(request.ch, callback);
        case message_actions_1.ACTIONS.pingPong:
  ...
  Agent.prototype._pingPong = function (callback) {
    var error = null;
    var message = {
      a: message_actions_1.ACTIONS.pingPong,
    };
  ...
  Agent.prototype._submit = function (collection, id, op, callback) {
    ...
    this.backend.submit(this, collection, id, op, null, function (err, ops, request) {
      ...
      if (request._fixupOps.length) ack[message_actions_1.ACTIONS.fixup] = request._fixupOps;
      if (err) {
  ...
  Agent.prototype._broadcastPresence = function (presence, callback) {
    ...
    subscriptionUpdater(presence.c, presence.d, function (error) {
      ...
      backend.trigger(backend.MIDDLEWARE_ACTIONS.receivePresence, agent, context, function (error) {
        if (error) return callback(error);
        var requests =
          presenceRequests[presence.ch] || (presenceRequests[presence.ch] = Object.create(null));
  ...
  Agent.prototype._subscribeDocVersion = function (collection, id, callback) {
    ...
    this.backend.subscribe(this, collection, id, null, function (error, stream, snapshot) {
      if (error) return callback(error);
      var versions =
        latestDocVersions[collection] || (latestDocVersions[collection] = Object.create(null));
      versions[id] = snapshot.v;
      agent._subscribeMapToStream(
        agent.latestDocVersionStreams,
        collection,
        id,
        stream,
        function (op) {
  ...
  Agent.prototype._createPresence = function (request) {
    return {
      a: message_actions_1.ACTIONS.presence,
      ch: request.ch,
      src: this._src(),
  ...
  Agent.prototype._handlePresenceData = function (presence) {
    ...
    if (presence.r)
      return this.send({ a: message_actions_1.ACTIONS.presenceRequest, ch: presence.ch });
    var backend = this.backend;
    var context = {
      collection: presence.c,
      presence: presence,
  ...
  Agent.prototype._checkFirstMessage = function (request) {
    ...
    if (request.a === message_actions_1.ACTIONS.handshake) {
      this._handshakeReceived = true;
      if (this._firstReceivedMessage.a !== message_actions_1.ACTIONS.handshake) {
...
function createClientOp(request, clientId) {
  ...
  return 'op' in request
    ? new EditOp(src, request.seq, request.v, request.op, request.x)
    : request.create
      ? new CreateOp(src, request.seq, request.v, request.create, request.x)
      : request.del
        ? new DeleteOp(src, request.seq, request.v, request.del, request.x)
        : undefined;
}
