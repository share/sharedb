...
Agent.prototype._open = function() {
  ...
  this.stream.on('data', function(chunk) {
    ...
    if (typeof chunk !== 'object') {
      var err = {code: ERROR_CODE.ERR_MESSAGE_BADLY_FORMED, message: 'Received non-object message'};
...
Agent.prototype._handleMessage = function(request, callback) {
  try {
    ...
    if (errMessage) return callback({code: ERROR_CODE.ERR_MESSAGE_BADLY_FORMED, message: errMessage});
    ...
    switch (request.a) {
      ...
      case 'op':
        ...
        if (!op) return callback({code: ERROR_CODE.ERR_MESSAGE_BADLY_FORMED, message: 'Invalid op message'});
