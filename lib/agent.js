...
Agent.prototype._cleanup = function() {
  ...
  // The channels that we want to let know that the agent
  // is no longer subscribed to.
  const channelsToInform = []

  // Clean up doc subscription streams
  for (var collection in this.subscribedDocs) {
    var docs = this.subscribedDocs[collection];
    for (var id in docs) {
      channelsToInform.push(this.backend.getDocChannel(collection, id))
  ...
  if (channelsToInform.length) {
    this.backend.notifyUnsubscribe(this.clientId, channelsToInform)
  }
};
...
Agent.prototype._unsubscribe = function(collection, id, callback) {
  ...
  if (stream) {
    stream.destroy();
    const docChannel = this.backend.getDocChannel(collection, id)
    this.backend.notifyUnsubscribe(this.clientId, [docChannel])
  }
  process.nextTick(callback);
};
