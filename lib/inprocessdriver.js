...
InprocDriver.prototype.atomicSubmit = function(cName, docName, opData, options, callback) {
  ...
  if (dirtyData) {
    for (var listName in dirtyData) {
      if (!this.dirtyLists[listName]) this.dirtyLists[listName] = [];

      this.dirtyLists[listName].push(dirtyData[listName]);
    }
  }
  var self = this;
...
InprocDriver.prototype.postSubmit = function(cName, docName, opData, snapshot) {
  ...
  this.subscribers.emit(cName, copy, cName);
};
