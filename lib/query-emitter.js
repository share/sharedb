...
QueryEmitter.prototype.update = function(op) {
  ...
  // and send it through immediately if it does. This means that the client
  // may see the result of the op get reflected before the query results
  // update. This might prove janky, since a doc could get deleted before it
  // is removed from the results, for example. However, it will mean that ops
  // which don't end up changing the results are received sooner.
  //
  // Alternatively, we could send the op message only after the query has
  // updated, but we do need to send the op even if the document no longer
  // matches the query. If client-side filters are applied to the model to
  // figure out which documents to render in a list, we will want the op that
  // removed the doc from the query
  if (this.ids.indexOf(id) > -1) {
    this.onOp(op);
    afterUpdate = this._defaultCallback;
  } else {
    // If the op's id doesn't match the query before updating, we need to
    // check again after, since this op might cause the query to match
    afterUpdate = function(err) {
      if (err) emitter.onError(err);
      if (emitter.ids.indexOf(id) > -1) emitter.onOp(op);
    };
  }

  // Ignore if the database or user function says we don't need to poll
  try {
    if (this.db.skipPoll(this.collection, id, op, this.query)) return afterUpdate();
    if (this.skipPoll(this.collection, id, op, this.query)) return afterUpdate();
  } catch (err) {
    console.error('Error evaluating skipPoll:', err.stack || err, this.collection, id, op, this.query);
