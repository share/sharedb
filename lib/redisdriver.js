...
RedisDriver.prototype.postSubmit = function(cName, docName, opData, snapshot, options) {
  ...
  if (options && !options.suppressCollectionPublish) {
    var collectionChannel = this._prefixChannel(cName);
    this.redis.publish(collectionChannel, msg);
    if (options.publishChannels) {
      // Sort and compare to lastChannel to remove any duplicate channels. This means that
      // code passing in the channels doesn't have to be careful about checking for dupes,
      // and we can naively combine the pre and post operation channels, which are going
      // to be the same most of the time
      options.publishChannels.sort();
      var lastChannel;
      for (var i = 0; i < options.publishChannels.length; i++) {
        var channel = options.publishChannels[i];
        if (channel === lastChannel) continue;
        lastChannel = channel;
        var prefixed = this._prefixChannel(channel);
        this.redis.publish(prefixed, msg);
      }
    }
  }
...
// Subscribe to changes on a collection or other channel. This is used by
// livedb's polling query code.
RedisDriver.prototype.subscribeChannel = function(channel, callback) {
  if (this.sdc) this.sdc.increment('livedb.subscribe.channel');
