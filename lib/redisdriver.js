...
RedisDriver.prototype.atomicSubmit = function(cName, docName, opData, options, callback) {
  ...
  var submitCallback = function(err, result) {
  ...
  var redisSubmitDelay = options && options.redisSubmitDelay;

  if (redisSubmitDelay || redisSubmitDelay === 0) {
    setTimeout(function() {
      self._redisSubmitScript(cName, docName, opData, dirtyData, null, submitCallback);
    }, redisSubmitDelay);
  } else {
    this._redisSubmitScript(cName, docName, opData, dirtyData, null, submitCallback);
  }
