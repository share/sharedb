...
RedisDriver.prototype.subscribeCollections = function(collections, callback) {
...
RedisDriver.prototype._subscribeChannels = function(channels, v, callback) {
  ...
  // Version number of the next op we expect to see. Be careful - this is modified & used elsewhere
  // too.
  stream._v = v;
  ...
  function pushOp(data) {
    if (stream._v && data.v) {
      if (data.v >= stream._v) {
        // data.v will usually be == stream._v, except if we're subscribing
