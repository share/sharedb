...
RedisDriver.prototype._unprefixChannel = function(channel) {
...
RedisDriver.prototype.postSubmit = function(cName, docName, opData, snapshot, options) {
  ...
  if (options && !options.suppressCollectionPublish) {
    ...
    for (var i = 0; i < publishChannels.length; ++i) {
      // TODO noansknv: If snapshot already has all the needed information, maybe we don't need to use
      // the middleware for subs?
      this.redis.publish(this._prefixChannel(publishChannels[i]), JSON.stringify(opData));
    }
  }
...
RedisDriver.prototype._subscribeChannels = function(channels, v, callback) {
  ...
  // Unprefixes database name from the channel and adds it to the message if applicable.
  var listener = function(msgChannel, data) {
    if (channels.indexOf(self._unprefixChannel(msgChannel)) === -1) return;
