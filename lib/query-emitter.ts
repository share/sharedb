...
class QueryEmitter {
  ...
  public backend:any;
    ...
    public fields:any;
    public options:any;
    public snapshotProjection:any;
    public stream:any;
    public ids:any;
    public extra:any;
    public skipPoll:any;
    public canPollDoc:any;
    public pollDebounce:any;
    public pollInterval:any;
    public _polling:any;
    public _pendingPoll:any;
    public _pollDebounceId:any;
    public _pollIntervalId:any;
    public _defaultCallback:any;

  constructor(request, stream, ids, extra) {
...
// Start processing events from the stream
  ...
  public _emitTiming(action, start) {
  ...
  public queryPoll(callback?) {
    ...
    this.db.queryPoll(this.collection, this.query, this.options, function (err, ids, extra) {
      ...
      if (idsDiff.length) {
        emitter.ids = ids;
        var inserted = QueryEmitter.getInserted(idsDiff);
        if (inserted.length) {
          emitter.db.getSnapshotBulk(emitter.collection, inserted, emitter.fields, null, function (err, snapshotMap) {
            if (err) return emitter._finishPoll(err, callback, pending);
            emitter.backend._sanitizeSnapshotBulk(emitter.agent, emitter.snapshotProjection,
                emitter.collection, snapshotMap, function (err) {
              if (err) return emitter._finishPoll(err, callback, pending);
              emitter._emitTiming('queryEmitter.pollGetSnapshotBulk', start);
              var diff = QueryEmitter.mapDiff(idsDiff, snapshotMap);
              emitter.onDiff(diff);
  ...
  public _finishPoll(err, callback, pending) {
    this._polling = false;
  ...
  public queryPollDoc(id, callback) {
    var emitter = this;
    var start = Date.now();
...
// are dropped.
  public onError(...args:any[]) {
    throw new ShareDBError(5018, 'Required QueryEmitter listener not assigned');
  };

  public onDiff(...args:any[]) {
    throw new ShareDBError(5018, 'Required QueryEmitter listener not assigned');
  };

  public onExtra(...args:any[]) {
    throw new ShareDBError(5018, 'Required QueryEmitter listener not assigned');
  };

  public onOp(...args:any[]) {
    throw new ShareDBError(5018, 'Required QueryEmitter listener not assigned');
  };

  static getInserted(diff) {
    var inserted:any = [];
  ...
  static mapDiff(idsDiff, snapshotMap) {
    var diff:any = [];
    for (var i = 0; i < idsDiff.length; i++) {
      var item:any = idsDiff[i];
      if (item instanceof arraydiff.InsertDiff) {
        var values:any = [];
        for (var j = 0; j < item.values.length; j++) {
