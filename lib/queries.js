  ...
  Livedb.prototype.queryPoll = function(index, query, options, callback) {
    ...
    this.driver.subscribeChannels(channels, function(err, stream) {
      ...
      livedb._dbQuery(db, index, query, {mode: 'initial'}, function(err, resultSet) {
        ...
        var queryEmitter = new QueryEmitter(index, query, stream, docNames, extra);
...
function readStreamItem(livedb, db, options, queryEmitter, collection, pollQuery, data) {
  ...
  if (!db.queryDoc) {
    ...
    console.warning('Ignoring _dbQueryDoc', queryEmitter.index, queryEmitter.query);
    return;
  }
...
function QueryEmitter(index, query, stream, docNames, extra) {
  this.index = index;
  this.query = query;
  this.stream = stream;
