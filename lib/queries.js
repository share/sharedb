...
module.exports = function(ShareDB) {
  ...
  ShareDB.prototype._queryStatsPrefix = function(options) {
    return 'query.' + (options.backend || 'default') + '.';
  };

  ShareDB.prototype._getQueryDb = function(options) {
    return (options.backend) ? this.extraDbs[options.backend] : this.db;
  };

  ShareDB.prototype._checkQueryDb = function(db) {
    if (!db) return 'Backend not found';
    if (!db.query) return 'Backend does not support queries';
  };

  ShareDB.prototype.queryFetch = function(index, query, options, callback) {
  ...
  ShareDB.prototype.querySubscribe = function(index, query, options, callback) {
    if (typeof options === 'function') {
      callback = options;
      options = {};
    }
