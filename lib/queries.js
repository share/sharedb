...
module.exports = function(Livedb) {
  ...
  //  liveness.
  // channelPostfix: postfix added to channel to scope down the subscriptions
  ...
  Livedb.prototype.queryPoll = function(index, query, opts, callback) {
    ...
    // Map from target channel name to projected channel name if there are any projections.
    var channel = index;

    var projection = this.projections[channel] && this.projections[channel].target;
    if (projection) {
      if (opts.channelPostfix) {
        channel = projection + ' ' + opts.channelPostfix;
      } else {
        channel = projection;
      }
    }
    ...
    this.driver.subscribeChannels([channel], function(err, stream) {
