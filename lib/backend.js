...
Backend.prototype._getSnapshot = function (agent, index, id, version, timestamp, callback) {
  ...
  this._getOps(agent, index, id, 0, version, options, function (error, ops) {
    ...
    for (var index = 0; index < ops.length; index++) {
      ...
      if (timestampIsNumber && fetchedTimestamp > timestamp) {
        var previousOp = ops[index - 1];
        fetchedTimestamp = previousOp ? previousOp.m.ts : undefined;
        fetchedVersion = previousOp ? previousOp.v : undefined;
        break;
      }
