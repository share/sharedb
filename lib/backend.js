...
var Backend = /** @class */ (function (_super) {
    ...
    function Backend(options) {
        ...
        if (!(_this instanceof Backend))
            return new Backend(options);
        if (!options)
            options = {};
        _this.db = options.db || new MemoryDB();
        _this.pubsub = options.pubsub || new MemoryPubSub();
        // This contains any extra databases that can be queried
        _this.extraDbs = options.extraDbs || {};
        // Map from projected collection -> {type, fields}
        _this.projections = {};
        _this.suppressPublish = !!options.suppressPublish;
        _this.maxSubmitRetries = options.maxSubmitRetries || null;
        // Map from event name to a list of middleware
        _this.middleware = {};
        // The number of open agents for monitoring and testing memory leaks
        _this.agentsCount = 0;
        _this.remoteAgentsCount = 0;
        return _this;
    }
    ...
    ;
    Backend.destroyStreams = function (streams) {
    ...
    Backend.prototype._triggerQuery = function (agent, index, query, options, callback) {
        ...
        backend.trigger('query', agent, request, function (err) {
            ...
            if (!request.db)
                return callback({ code: 4003, message: 'DB not found' });
    ...
    ;
    Backend.pluckIds = function (snapshots) {
        var ids = [];
