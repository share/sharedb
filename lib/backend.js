...
Backend.prototype.getOps = function(agent, index, id, from, to, callback) {
  return this._getOps(agent, index, id, from, to, null, callback);
};

// Non inclusive - gets ops from [from, to). Ie, all relevant ops. If to is
// not defined (null or undefined) then it returns all ops.
Backend.prototype._getOps = function(agent, index, id, from, to, options, callback) {
  var start = Date.now();
  ...
  backend.db.getOps(collection, id, from, to, options, function(err, ops) {
    if (err) return callback(err);
...
Backend.prototype.getSnapshot = function(agent, index, id, version, timestamp, callback) {
  var backend = this;
  this._getSnapshot(agent, index, id, version, timestamp, function (error, snapshot) {
...
Backend.prototype._getSnapshot = function (agent, index, id, version, timestamp, callback) {
  ...
  version = (typeof version === 'number' && isFinite(version)) ? version + 1 : null;
  var options = { metadata: true };
  var timestampIsNumber = typeof timestamp === 'number';

  if (timestampIsNumber && timestamp > Date.now()) {
    return callback({ code: 4025, message: 'Requested timestamp must be in the past' });
  }
  ...
  this._getOps(agent, index, id, 0, version, options, function (error, ops) {
    ...
    var type = null;
    var snapshot;
    var fetchedTimestamp = 0;
    var fetchedVersion = 0;

    for (var index = 0; index < ops.length; index++) {
      var op = ops[index];

      if (timestampIsNumber && op.m.ts > timestamp) {
        break;
      }
    ...
    if (requestedVersion > fetchedVersion) {
