...
function SubmitRequest(backend, agent, index, id, op, presence, options) {
...
SubmitRequest.prototype.submit = function(callback) {
  ...
  backend.db.getSnapshot(collection, id, fields, null, function(err, snapshot) {
    ...
    backend.db.getOpsToSnapshot(collection, id, from, snapshot, null, function(err, ops) {
      ...
      backend.trigger('transformOp', agent, request, function(err) {
        if (err) return callback(err);
  ...
  backend.trigger('commit', this.agent, this, function(err) {
    ...
    backend.db.commit(request.collection, request.id, request.op, request.snapshot, request.options, function(err, succeeded) {
      if (err || !succeeded) {
        ...
        return request.retry(callback, err);
      }
      if (!request.suppressPublish) {
        ...
        op.d = request.id;
        op.m = undefined;
        if (request.presence) op.pr = request.presence
        ...
        backend.trigger('publish', this.agent, request, function(err) {
          if (err) return callback(err);
...
SubmitRequest.prototype.retry = function(callback, err) {
  this.retries++;
  if (this.maxRetries != null && this.retries > this.maxRetries) {
    return callback(this.maxRetriesError(err));
  }
...
SubmitRequest.prototype.maxRetriesError = function(err) {
  if (err) {
    return {code: 5004, message: `Op submit failed. Maximum submit retries exceeded. Attempt failed with following error ${err}`};
  } else {
