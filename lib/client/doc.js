...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  ...
  this._setType(type);
  this.data = (this.type && this.type.serialize) ? this.type.deserialize(snapshot.data) : snapshot.data
  this.emit('load');
  callback && callback();
};
...
Doc.prototype._otApply = function(op, source) {
  ...
  if (op.create) {
    ...
    var data = (this.type.serialize) ? this.type.deserialize(op.create.data) : op.create.data;
    this.data = this.type.create(data);
...
Doc.prototype._tryCompose = function(op) {
  ...
  if (last.create && op.op) {
    var data = (this.type.serialize) ? this.type.deserialize(last.create.data) : last.create.data;
    data = this.type.apply(data, op.op);
    last.create.data = (this.type.serialize) ? this.type.serialize(data) : data;
    return last;
  }
...
// object, depending on the type.  If the type supports serizliation then the
// initial data will be serialized.
//
// @param data  initial
// @param type  OT type
// @param options  {source: ...}
...
Doc.prototype.create = function(data, type, options, callback) {
  ...
  if (type.serialize) {
    data = type.serialize(data);
  }
  var op = {create: {type: type.uri, data: data}};
  var source = options && options.source;
  this._submit(op, source, callback);
};
