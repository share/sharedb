...
Doc.prototype._hardRollback = function(err) {
  ...
  this._emitPresence(changedSrcList);
...
Doc.prototype.submitPresence = function (data, callback) {
  if (data != null) {
    if (!this.type) {
      var err = new ShareDBError(4015, 'Cannot submit presence. Document has not been created. ' + this.collection + '.' + this.id);
      if (callback) return callback(err);
      return this.emit('error', err);
    }

    if (!this.type.createPresence || !this.type.transformPresence) {
      var err = new ShareDBError(4024, 'Cannot submit presence. Document\'s type does not support presence. ' + this.collection + '.' + this.id);
      if (callback) return callback(err);
      return this.emit('error', err);
    }
  ...
  if (!this.pendingPresence) this.pendingPresence = [];
  if (callback) this.pendingPresence.push(callback);
  this._setPresence('', data, true);
...
Doc.prototype._setPresence = function(src, data, emit) {
  ...
  if (emit) this._emitPresence([ src ]);
  return true;
};

Doc.prototype._emitPresence = function(srcList) {
  if (srcList && srcList.length > 0) {
    this.emit('presence', srcList);
  }
};
