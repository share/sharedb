...
var Doc = /** @class */ (function () {
  ...
  Doc.prototype._queueSubscribe = function (wantSubscribe, callback) {
    var lastRequest =
      this.pendingSubscribe[this.pendingSubscribe.length - 1] || this.inflightSubscribe;
      ...
      if (!this.type) {
        if (this._isInHardRollback) {
          var err = new ShareDBError(
            ...
            'Cannot submit op. Document is performing hard rollback. ' +
              this.collection +
              '.' +
              this.id,
          );
  ...
  Doc.prototype._opAcknowledged = function (message) {
    ...
    if (message[message_actions_1.ACTIONS.fixup]) {
      for (var i = 0; i < message[message_actions_1.ACTIONS.fixup].length; i++) {
        var fixupOp = message[message_actions_1.ACTIONS.fixup][i];
