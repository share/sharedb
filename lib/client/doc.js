...
// By default, this implementation defensively clones the document data
// to guard against mutations. You can opt out of cloning by passing `true`
// as the optional `doNotClone` argument.
//
// @param doNotClone (optional) If `true`, the document data is not cloned. This can be used
//        as a performance optimization, but be sure that your code does not mutate the data.
Doc.prototype.toSnapshot = function(doNotClone) {
  return {
    v: this.version,
    data: doNotClone ? this.data : util.clone(this.data),
    type: this.type.uri
  };
};
