...
Doc.prototype._sendOp = function() {
  ...
  if (!this.inflightOp) {
    ...
    // TODO: Handle create/del?
    for (var id in this._heldOps) {
      var heldOp = this._heldOps[id];
      // TODO: Any way to do this for non-invertible types?
      var inverse = heldOp.type.invert(heldOp.op);
      this.inflightOp.op = this.inflightOp.type.transform(this.inflightOp.op, inverse, 'left');
    }
  }
...
Doc.prototype._pushOp = function(op, callback) {
  ...
  for (var id in this._heldOps) {
    var heldOp = this._heldOps[id];
    heldOp.op = op.type.transform(heldOp.op, op.op, 'right');
  }
};
