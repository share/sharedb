...
function Doc(connection, collection, id) {
  ...
  if (connection.Presence) {
    // TODO don't decorate
    // Expose presence-related methods on the Doc prototype.
    Object.assign(Doc.prototype, connection.Presence);

    delete Doc.prototype.hardRollbackPresence;
    delete Doc.prototype._initializePresence;
    delete Doc.prototype._processReceivedPresence;
    delete Doc.prototype.processAllReceivedPresence;
    delete Doc.prototype._transformPresence;
    delete Doc.prototype.transformAllPresence;
    delete Doc.prototype.pausePresence;
    delete Doc.prototype.cacheOp;
    delete Doc.prototype._flushPresence;
    delete Doc.prototype._emitPresence;
    delete Doc.prototype.submitPresence;
    delete Doc.prototype._destroyPresence;
    delete Doc.prototype._setPresence;

    // TODO move this outside
    Doc.prototype.submitPresence = function (data, callback) {
      this.presence.submitPresence(data, callback);
    };

    // TODO move this with other _handle... definitions.
    ...
    // If this.presence is falsy (undefined), it means that
    // the enablePresence flag was not passed into the ShareDB constructor,
    // so the presence features should be disabled.
    //
    // TODO convert to constructor.
    this.presence = connection.Presence._initializePresence();

    this.presence.doc = this;
    Object.assign(this.presence, connection.Presence);
  ...
  doc.whenNothingPending(function() {
    if (doc.wantSubscribe) {
      doc.unsubscribe(function(err) {
        ...
        if (doc.presence) doc._destroyPresence();
...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  ...
  this.data = (this.type && this.type.deserialize) ?
    ...
    snapshot.data;
  this.emit('load');
  if (this.presence) this.presence.processAllReceivedPresence();
  callback && callback();
};
...
Doc.prototype._handleOp = function(err, message) {
  ...
  this.version++;
  if (this.presence) this.presence.cacheOp({
    src: message.src,
    time: Date.now(),
...
Doc.prototype._onConnectionStateChanged = function() {
  if (this.connection.canSend) {
    ...
    if (this.inflightUnsubscribe.length) {
      ...
      this.inflightUnsubscribe = [];
      if (this.presence) this.presence.pausePresence();
      callEach(callbacks);
    } else {
      if (this.presence) this.presence.pausePresence();
    }
  ...
  if (op.op) {
    ...
    if (!source && this.type === types.defaultType && op.op.length > 1) {
      ...
      for (var i = 0; i < op.op.length; i++) {
        ...
        if (this.presence) this.presence.transformAllPresence(componentOp);
    ...
    if (this.presence) this.presence.transformAllPresence(op);
...
Doc.prototype._hardRollback = function(err) {
  ...
  var pendingPresence = this.presence ? this.presence.hardRollbackPresence() : [];
...
// Expose callEach to presence methods via Doc prototype.
Doc.prototype._callEach = callEach;
