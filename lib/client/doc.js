...
Doc.prototype._otApply = function(op, options) {
  ...
  if (op.op) {
    ...
    var undoOp = options && options.undoOp && options.undoOp.op || null;
    ...
    if (!source && this.type === types.defaultType && op.op.length > 1) {
      ...
      for (var i = 0; i < op.op.length; i++) {
        ...
        this._applyOp(componentOp.op, undoOp, source, undoable, fixUp);
    ...
    this._applyOp(op.op, undoOp, source, undoable, fixUp);
...
Doc.prototype._applyOp = function(op, undoOp, source, undoable, fixUp) {
  if (undoOp == null && (undoable || fixUp)) {
    if (this.type.applyAndInvert) {
      var result = this.type.applyAndInvert(this.data, op);
      this.data = result[0];
      undoOp = result[1];
    } else {
      this.data = this.type.apply(this.data, op);
      undoOp = this.type.invert(op);
    }
  } else {
    this.data = this.type.apply(this.data, op);
  }
...
//   It also updates UndoManagers in such a way that undo/redo will "skip" over the current state of the snapshot.
//   This feature requires the OT type to implement `compose`.
