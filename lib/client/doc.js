...
Doc.prototype._handleOp = function(err, message) {
  if (err) {
    if (err.code === ERROR_CODE.ERR_NO_OP && message.seq === this.inflightOp.seq) {
      ...
      return this.fetch(this._clearInflight.bind(this));
    }
    if (this._hasInflight()) {
...
Doc.prototype.flush = function() {
  ...
  // TODO: Transactions should wait for all docs to only have transaction ops left in pendingOps
  // Transactions will be flushed as one call separately
...
Doc.prototype._submit = function(op, source, callback) {
  ...
  var transactionError = this._setTransaction(op.transaction);
  if (transactionError) return this._callbackOrEmitError(error);

  try {
    this._pushOp(op, source, callback);
    this._otApply(op, source);
  } catch (error) {
    // TODO: Transactions should abort and rollback all docs in transaction
    return this._hardRollback(error);
  }
...
Doc.prototype.submitOp = function(component, options, callback) {
  ...
  options = options || {};
  var op = {op: component};
  if (options.transaction) op.transaction = this._normalizeTransactionId(options.transaction);
...
Doc.prototype._rollback = function(err) {
  ...
  var ops = [];
  if (this.inflightOp) ops.push(this.inflightOp);
  if (this._transaction) {
    for (var op of this.pendingOps) {
      if (op.transaction !== this._transaction.id) break;
      ops.push(op);
    }
  }
  ...
  if (err.code === ERROR_CODE.ERR_OP_SUBMIT_REJECTED) {
    return this._clearInflight(null);
  }

  this._clearInflight(err);
};
...
Doc.prototype._hasInflight = function() {
...
Doc.prototype._clearInflightOp = function() {
  var inflightOp = this.inflightOp;
  this.inflightOp = null;
  return inflightOp.callbacks;
};

Doc.prototype._clearTransaction = function() {
  var transactionId = this._transaction.id;
  var callbacks = [];
  var i = 0;
  for (; i < this.pendingOps.length; i++) {
    var op = this.pendingOps[i];
    if (op.transaction !== transactionId) break;
    callbacks = callbacks.concat(op.callbacks);
  }

  this.pendingOps.splice(0, i);
  this._transaction = null;

  return callbacks;
};

Doc.prototype._setTransaction = function(transactionId) {
  if (transactionId == util.dig(this, '_transaction', 'id')) return;

  if (this._transaction) {
    return new Error('Transaction in progress. Must commit transaction before submitting any other ops');
  }

  var transaction = this.connection._transactions[transactionId];
  if (!transaction) throw new Error('Transaction not found');
  transaction._registerDoc(this);
};

Doc.prototype._normalizeTransactionId = function(transaction) {
  if (!transaction) return null;
  if (typeof transaction === 'string') return transaction;
  return transaction.id;
};

Doc.prototype._callbackOrEmitError = function(error, callback) {
