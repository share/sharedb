...
Doc.prototype._otApply = function(op, options) {
  ...
  if (op.op) {
    ...
    var fixUpUndoStack = options && options.fixUpUndoStack || false;
    var fixUpRedoStack = options && options.fixUpRedoStack || false;
    ...
    if (!source && this.type === types.defaultType && op.op.length > 1) {
      ...
      for (var i = 0; i < op.op.length; i++) {
        ...
        this.emit('before op', componentOp.op, source, operationType);
        this._applyOp(componentOp, undoOp, operationType, fixUpUndoStack, fixUpRedoStack);
        this.emit('op', componentOp.op, source, operationType);
      }
    ...
    this.emit('before op', op.op, source, operationType);
    // Apply the operation to the local data, mutating it in place
    this._applyOp(op, undoOp, operationType, fixUpUndoStack, fixUpRedoStack);
...
Doc.prototype._applyOp = function(op, undoOp, operationType, fixUpUndoStack, fixUpRedoStack) {
...
// @param [op.del]
// @param [op.create]
// @param options { source, skipNoop, operationType, undoOp, fixUpUndoStack, fixUpRedoStack }
...
Doc.prototype._submit = function(op, options, callback) {
  ...
  if (op.op) {
    ...
    var notFixedOperation = options.operationType !== OPERATION_TYPES.FIXED;
    var fixUpUndoStack = options.fixUpUndoStack;
    var fixUpRedoStack = options.fixUpRedoStack;
    var needsInvert = notFixedOperation || fixUpUndoStack || fixUpRedoStack;
    if (needsInvert && !this.type.invert && !this.type.applyAndInvert) {
