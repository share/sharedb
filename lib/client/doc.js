...
// @param data.data
// @param data.type
Doc.prototype.ingestData = function(data, callback) {
  if (!data) return callback && callback();

  // Throw away data if the doc is already created, since we need ops in order
  // to update the document
  if (this.type) {
    // If we got a snapshot for a version further along than the document is
    // currently, issue a fetch to get the latest ops and catch us up
    if (data.v > this.version) return this.fetch(callback);
