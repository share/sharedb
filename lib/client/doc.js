...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  ...
  if (typeof snapshot.v !== 'number') {
    var err = new ShareDBError(
      ERROR_CODE.MISSING_SNAPSHOT_VERSION,
...
Doc.prototype._handleOp = function(err, message) {
  if (err) {
    if (this.inflightOp) {
      ...
      if (err.code === ERROR_CODE.OP_SUBMIT_REJECTED) err = null;
...
function transformX(client, server) {
  ...
  if (server.del) {
    return new ShareDBError(ERROR_CODE.DOCUMENT_WAS_DELETED, 'Document was deleted');
  }
  if (server.create) {
    return new ShareDBError(ERROR_CODE.DOCUMENT_CREATED_REMOTELY, 'Document already created');
  }
  ...
  if (op.op) {
    if (!this.type) {
      ...
      throw new ShareDBError(
        ERROR_CODE.DOCUMENT_DOES_NOT_EXIST,
...
Doc.prototype._sendOp = function() {
  ...
  if (!op) {
    var err = new ShareDBError(ERROR_CODE.NO_OP_TO_SEND, 'No op to send on call to _sendOp');
    return this.emit('error', err);
  }
...
Doc.prototype.create = function(data, type, options, callback) {
  ...
  if (this.type) {
    var err = new ShareDBError(ERROR_CODE.DOCUMENT_ALREADY_CREATED, 'Document already exists');
    if (callback) return callback(err);
    return this.emit('error', err);
  }
...
Doc.prototype.del = function(options, callback) {
  ...
  if (!this.type) {
    var err = new ShareDBError(ERROR_CODE.DOCUMENT_DOES_NOT_EXIST, 'Document does not exist');
    if (callback) return callback(err);
    return this.emit('error', err);
  }
  var op = {del: true};
