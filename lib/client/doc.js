...
function Doc(connection, collection, id) {
  ...
  this.presence = connection.Presence
    ? new connection.Presence(this)
    : new DummyPresence();
...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  ...
  this.data = (this.type && this.type.deserialize) ?
    ...
    snapshot.data;
  this.emit('load');
  this.presence.processAllReceivedPresence();
  callback && callback();
};
  ...
  if (op.op) {
    ...
    if (!source && this.type === types.defaultType && op.op.length > 1) {
      ...
      for (var i = 0; i < op.op.length; i++) {
        ...
        this.presence.transformAllPresence(componentOp);
    ...
    this.presence.transformAllPresence(op);
...
Doc.prototype._opAcknowledged = function(message) {
  ...
  this.presence.cacheOp(this.inflightOp);
...
Doc.prototype._hardRollback = function(err) {
  ...
  var pendingPresence = this.presence.getPendingPresence();
  this.presence.hardRollbackPresence();
