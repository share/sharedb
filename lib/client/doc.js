 ...
 * - `before op (op, source, presence)` Fired before a partial operation is applied to the data.
 *   It may be used to read the old data just before applying an operation
 * - `op (op, source, presence)` Fired after every partial operation with this operation as the
 *   first argument
 * - `on presence (presence)` Fired when presence information is received independent of an op
 * - `unsub notification (clientId)` Fired when another client unsubscribes from this document
...
Doc.prototype._handlePresence = function(err, message) {
  if (err) {
    return this.emit('error', err);
  }
  ...
  this.emit('on presence', this._transformPresence(presence))
}

Doc.prototype._handleUnsubscribeNotification = function(err, message) {
  if (err) {
    return this.emit('error', err)
  }

  this.emit('unsub notification', message.src)
}

Doc.prototype._transformPresence = function(presence) {
  const ops = [this.inflightOp, ...this.pendingOps].filter(Boolean)
  if (ops.length && this.presenceTransformer) {
    return this.presenceTransformer(presence, ops)
  } else {
    return presence
  }
}
...
Doc.prototype._otApply = function(op, source) {
  if (op.op) {
    ...
    // Transform the presence only if it's a remote op
    const presence = op.pr && !source ? this._transformPresence(op.pr) : undefined
    const presenceWithSource =  presence ? {
      ...presence,
      src: op.src
    } : undefined
    ...
    this.emit('before op', op.op, source, presenceWithSource);
    ...
    this.emit('op', op.op, source, presenceWithSource);
    return;
  }
...
Doc.prototype._tryCompose = function(op) {
  ...
  if (last.op && op.op && this.type.compose) {
    ...
    // Use the most recent presence data for the op
    if (op.pr) {
      last.pr = op.pr
    }
    return last;
...
Doc.prototype.submitOp = function(component, options, callback) {
  ...
  var op = {op: component};
  // Clear any pending presence since we'll be sending the latest presence along with the op
  this.pendingPresence = null
  // Append any transient presence data to the top level op
  if (options && options.pr) {
    op.pr = options.pr
  }
...
Doc.prototype.sendPresence = function(presence) {
  if (this.pendingOps.length) {
    // If there are pending ops, attach the updated presence to the most recent pending op
    const lastOp = this.pendingOps[this.pendingOps.length - 1]
    lastOp.pr = presence
    this.pendingPresence = null
  } else if (!this.hasPending()) {
    // If there is nothing pending (no inflight or pending ops), then broadcast the presence
    this.pendingPresence = null
    this.connection.sendPresence(this, presence)
  } else {
    // If there are inflight ops, wait till they are acknowledged before sending presence
    this.pendingPresence = presence
    this.whenNothingPending(() => {
      if (this.pendingPresence) {
        // Send pendingPresence if still necessary
        // This might not be necessary if an op has been submitted with newer presence before the
        // 'whenNothingPending' callback has fired
        this.connection.sendPresence(this, this.pendingPresence)
        this.pendingPresence = null
      }
    })
  }
...
// Set a function (presence, ops) => presence that transforms
// the provided presence by the provided ops
// This is needed when you receive presence while there are pending ops
Doc.prototype.setPresenceTransformer = function(presenceTransformer) {
  this.presenceTransformer = presenceTransformer
}
...
Doc.prototype._rollback = function(err) {
  ...
  /**
   * 'json0' is technically an invertible type but we register 'rich-text' as a subtype
   * Since 'rich-text' is not invertible, it makes the parent type not invertible as well
   */
