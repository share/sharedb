...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  ...
  // If the doc is already created, we cannot use the ingested snapshot and
  // need ops in order to update the document
  if (this.type) {
...
Doc.prototype._handleOp = function(err, message) {
  ...
  if (this.version == null || message.v > this.version) {
    ...
    //
    // Only send a new fetch command if no fetches or subscribes are inflight,
    // which will act as a natural debouncing so we don't send multiple fetch
  ...
  if (this.inflightOp) transformX(this.inflightOp, message);

  for (var i = 0; i < this.pendingOps.length; i++) {
    transformX(this.pendingOps[i], message);
  }
...
function transformX(client, server) {
  ...
  if (client.create) {
    var err = new Error('Cannot transform client create op. ' + this.collection + '.' + this.id);
    return this.emit('error', err);
  }
...
Doc.prototype._rollback = function(err) {
  ...
  var doc = this;
  this.fetch(function() {
    var called = callEach(op.callbacks, err);
