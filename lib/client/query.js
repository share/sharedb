...
Query.prototype._handleResponse = function(err, data, extra) {
  ...
  if (Array.isArray(data)) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var doc = this.connection.get(docData.c || this.collection, docData.d);
      wait++;
      doc.ingestData(docData, finish);
    ...
    for (var id in data) {
      var docData = data[id];
      var doc = this.connection.get(docData.c || this.collection, id);
      wait++;
      doc.ingestData(docData, finish);
    }
  }
...
Query.prototype._handleDiff = function(err, diff, extra) {
  ...
  if (diff) {
    ...
    for (var i = 0; i < diff.length; i++) {
      var d = diff[i];
      if (d.type === 'insert') d.values = this._ingestResults(d.values);
    }
