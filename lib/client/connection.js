...
Connection.prototype.bindToSocket = function(socket) {
  ...
  socket.onmessage = function(event) {
    ...
    try {
      ...
      connection.emit('error', err, request.data);
    }
  };
  ...
  switch (message.a) {
    case 'init':
      ...
      if (message.protocol !== 1) {
        var err = new Error('Invalid protocol version');
        return this.emit('error', err);
      }
      if (types.map[message.type] !== types.defaultType) {
        var err = new Error('Invalid default type');
        return this.emit('error', err);
      }
    ...
    case 'qf':
      ...
      if (query) query._handleFetch(message.error, message.data, message.extra);
      return;
    case 'qs':
      var query = this.queries[message.id];
      if (query) query._handleSubscribe(message.error, message.data, message.extra);
      return;
    case 'qu':
    ...
    case 'q':
      ...
      var query = this.queries[message.id];
      if (!query) return;
      if (message.error) return query._handleError(message.error);
    ...
    case 'f':
      ...
      if (doc) doc._handleFetch(message.error, message.data);
      return;
    case 's':
      var doc = this.getExisting(message.c, message.d);
      if (doc) doc._handleSubscribe(message.error, message.data);
      return;
    case 'u':
      var doc = this.getExisting(message.c, message.d);
      if (doc) doc._handleUnsubscribe(message.error);
      return;
    case 'op':
      var doc = this.getExisting(message.c, message.d);
      if (doc) doc._handleOp(message.error, message);
      return;
