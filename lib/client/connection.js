  ...
  switch (message.a) {
    case 'init':
      ...
      if (message.protocol !== 1) {
        err = new ShareDBError(ERROR_CODE.INVALID_PROTOCOL_VERSION, 'Invalid protocol version');
        return this.emit('error', err);
      }
      if (types.map[message.type] !== types.defaultType) {
        err = new ShareDBError(ERROR_CODE.INVALID_DEFAULT_TYPE, 'Invalid default type');
        return this.emit('error', err);
      }
      if (typeof message.id !== 'string') {
        err = new ShareDBError(ERROR_CODE.INVALID_CLIENT_ID, 'Invalid client id');
        return this.emit('error', err);
      }
      this.id = message.id;
