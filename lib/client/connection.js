...
Connection.prototype._setState = function(newState, reason) {
  ...
  for (var c in this.collections) {
    ...
    for (var docName in collection) {
      collection[docName]._onConnectionStateChanged();
    }
  }
  this.bulkEnd();
};

Connection.prototype.bulkStart = function() {
  if (!this.bulk) this.bulk = {};
};

Connection.prototype.bulkEnd = function() {
  if (this.bulk) {
...
Connection.prototype.sendSubscribe = function(doc) {
  ...
  if (this.bulk) {
    ...
    if (doc.version == null) {
      ...
      subscribe.push(doc.name);
    } else {
      var subscribeFrom = actions.subscribeFrom || (actions.subscribeFrom = []);
      subscribeFrom[doc.name] = doc.version;
    }
  } else {
    // Send single doc subscribe message
    var msg = {a: 's', c: doc.collection, d: doc.name, v: doc.version};
    this.send(msg);
  }
};
...
Connection.prototype.sendFetch = function(doc) {
  ...
  if (this.bulk) {
    ...
    if (doc.version == null) {
      ...
      fetchFrom[doc.name] = doc.version;
    }
  } else {
    // Send single doc fetch message
    var msg = {a: 'f', c: doc.collection, d: doc.name, v: doc.version};
    this.send(msg);
  }
};
...
Connection.prototype.sendUnsubscribe = function(doc) {
  ...
  if (this.bulk) {
    ...
    unsubscribe.push(doc.name);
  } else {
    // Send single doc unsubscribe message
    var msg = {a: 'u', c: doc.collection, d: doc.name};
    this.send(msg);
  }
};
...
Connection.prototype.get = function(collection, name, data) {
  ...
  var doc = docs[name];
  if (!doc) {
    doc = docs[name] = new Doc(this, collection, name);
    this.emit('doc', doc);
  }
