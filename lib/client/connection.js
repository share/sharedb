...
Connection.prototype._setState = function(newState, reason) {
  ...
  for (var id in this.snapshotRequests) {
    var snapshotRequest = this.snapshotRequests[id];
...
Connection.prototype.hasPending = function() {
  return !!(
    ...
    this._firstSnapshotRequest(hasPending)
  );
};
 ...
 * @param version (optional) - the version number to fetch. If an exact version match is not made, then the next lower
...
Connection.prototype.getSnapshot = function(collection, id, version, callback) {
  ...
  if (!this._isNumberOrNully(version)) {
    return callback({ code: 4024, message: 'Invalid version' });
  }

  var snapshotRequest = new SnapshotRequest(this, collection, id, callback);
  snapshotRequest.version = version;
  this.snapshotRequests[snapshotRequest.requestId] = snapshotRequest;
 ...
 * @param collection - the collection name of the snapshot
 * @param id - the ID of the snapshot
 * @param version (optional) - the timestamp of the snapshot to fetch. If an exact timestamp match is not made, then the
 *   next lower version is returned. ie if ops were submitted at 02:00 and 03:00, then asking for a Date at 02:30 will
 *   return the 02:00 version.
 * @param callback - (error, snapshot) => void, where snapshot takes the same schema as getSnapshot (see above)
 */
Connection.prototype.getSnapshotAtTime = function (collection, id, timestamp, callback) {
  if (typeof timestamp === 'function') {
    callback = timestamp;
    timestamp = null;
  }

  if (!this._isNumberOrNully(timestamp)) {
    return callback({ code: 4025, message: 'Invalid timestamp' });
  }

  var snapshotRequest = new SnapshotRequest(this, collection, id, callback);
  snapshotRequest.timestamp = timestamp;
  this.snapshotRequests[snapshotRequest.requestId] = snapshotRequest;
  snapshotRequest.send();
};
...
Connection.prototype._isNumberOrNully = function (number) {
  if (number == null) {
    return true;
  }
