...
function UndoManager(connection, options) {
  ...
  // The "source" value of undoable operations. If null or undefined, it works for all operations.
...
UndoManager.prototype.destroy = function() {
  this.connection.removeUndoManager(this);
};
...
UndoManager.prototype._updateStacksUndoable = function(doc, op, undoOp) {
  ...
  if (
    ...
    this._undoStack.push(new Item(undoOp.op, doc));

  } else if (doc.type.composeSimilar) {
    var lastOp = getLast(this._undoStack);
    var composedOp = doc.type.composeSimilar(undoOp.op, lastOp.op);
    if (composedOp != null) {
    ...
    var lastOp = getLast(this._undoStack);
    var composedOp = doc.type.compose(undoOp.op, lastOp.op);
    setLast(this._undoStack, new Item(composedOp, doc));
...
UndoManager.prototype._updateStacksUndo = function(doc, op, undoOp) {
  if (!doc.type.isNoop || !doc.type.isNoop(undoOp.op)) {
    this._redoStack.push(new Item(undoOp.op, doc));
  }
  this._previousUndoableOperationTime = -Infinity;
};
...
UndoManager.prototype._updateStacksFixed = function(doc, op, undoOp, fixUp) {
  ...
  if (fixUp && undoOp && doc.type.compose) {
    ...
    this._undoStack = this._transformStack(this._undoStack, doc, op.op);
    this._redoStack = this._transformStack(this._redoStack, doc, op.op);
  }
};
