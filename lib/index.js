...
// By default we publish only to collection.
// TODO noansknv: Is this agnostic enough?
function defaultPublishChannels(cName, opData, snapshot) { return [cName] };
...
function Livedb(options) {
  ...
  this.getPublishChannels = options.getPublishChannels || defaultPublishChannels;
};
        ...
        if (err === 'Transform needed') {
          ...
          self._writeSnapshotAfterSubmit(submitData.cName, submitData.docName, snapshot, opData, submitData.submitOptions, function(err) {
            ...
            var postSubmitOptions = {
              ...
              getPublishChannels: self.getPublishChannels // TODO noansknv: Is this agnostic enough?
            };

            if (self.driver.postSubmit) {
              self.driver.postSubmit(submitData.cName, submitData.docName, opData, snapshot, postSubmitOptions);
            }
