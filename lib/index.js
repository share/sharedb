...
Livedb.prototype._trySubmit = function(submitData) {
  ...
  this._lazyFetch(submitData.cName, submitData.docName, function(err, snapshot) {
    ...
    self.driver.getOps(submitData.cName, submitData.docName, from, to, function(err, ops) {
      ...
      var preChannels = self.getPublishChannels(submitData.cName, submitData.docName, opData, snapshot);
      ...
      var postChannels = self.getPublishChannels(submitData.cName, submitData.docName, opData, snapshot);
      ...
      submitOptions.suppressCollectionPublish = self.suppressCollectionPublish;
      ...
      self.driver.atomicSubmit(submitData.cName, submitData.docName, opData, submitOptions, function(err) {
        ...
        self._writeSnapshotAfterSubmit(submitData.cName, submitData.docName, snapshot, opData, submitOptions, function(err) {
          ...
          if (self.driver.postSubmit) self.driver.postSubmit(submitData.cName, submitData.docName, opData, snapshot, submitOptions);
...
Livedb.prototype._writeSnapshotAfterSubmit = function(cName, docName, snapshot, opData, options, callback) {
  ...
  this.snapshotDb.writeSnapshot(cName, docName, snapshot, function(err) {
    ...
    for (var name in self.extraDbs) {
      ...
      if (db.submit) {
        db.submit(cName, docName, opData, options, snapshot, self, (function(dbName) {
          return function(err) {
            if (err) {
              console.warn("Error updating db " + dbName + " " +
