...
//
// - publishOperationsToCollections  This is used to determine whether to publish submitted operations
//     to collections.
function Livedb(options) {
  ...
  this.publishOperationsToCollections = typeof options.publishOperationsToCollections === 'undefined' ? true : options.publishOperationsToCollections;
};
        ...
        self.driver.atomicSubmit(cName, docName, opData, options, function(err) {
          if (err === 'Transform needed') {
            ...
            self._writeSnapshotAfterSubmit(cName, docName, snapshot, opData, options, function(err) {
              ...
              if (self.driver.postSubmit)
                self.driver.postSubmit(cName,
                  docName,
                  opData,
                  snapshot, {
                  publishOperationsToCollections: self.publishOperationsToCollections
                });
  ...
  if (projection) {
    if (this.snapshotDb.getSnapshotProjected) {
      ...
      this.snapshotDb.getSnapshot(projection.target, docName, function(err, snapshot) {
        ...
        // console.log(projection, snapshot.type);
        if (snapshot) {
