...
Livedb.prototype.queryFetch = function(cName, query, opts, callback) {
  ...
  if (self.sdc) self.sdc.increment('livedb.db.query.initial');
  ...
  db.query(this, cName, query, function(err, resultSet) {
    if (self.sdc) {
      ...
      self.sdc.timing('livedb.query.fetch', Date.now() - start);
      self.sdc.timing('livedb.db.query', Date.now() - dbStart);
    }
...
Livedb.prototype.query = function(index, query, opts, callback) {
  ...
  this.subscribeChannels(channels, function(err, stream) {
    ...
    db.query(self, index, query, function(err, resultSet) {
      ...
      if (self.sdc) {
        self.sdc.timing('livedb.db.query', Date.now() - dbStart);
        self.sdc.increment('livedb.db.query.initial');
      }
      ...
      if (poll) {
        var runQuery = rateLimit(delay, function() {
          ...
          var dbStart = Date.now();
          db.query(self, index, query, function(err, newResultset) {
            if (err) return emitter.emit('error', new Error(err));

            if (self.sdc) {
              self.sdc.timing('livedb.db.query', Date.now() - dbStart);
              self.sdc.increment('livedb.db.query.poll');
            }
      ...
      if (self.sdc) self.sdc.timing('livedb.query.subscribe', Date.now() - start);
