...
function TrySubmitOptions(cName, docName, opData, submitOptions, projection, callback) {
    this.cName = cName;
...
Livedb.prototype.submit = function(cName, docName, opData, submitOptions, callback) {
  ...
  var options = new TrySubmitOptions(
    cName,
    docName,
    opData,
    submitOptions,
  ...
  if (this.submitMap[opData.src]) {
    this.submitMap[opData.src].push(options);
  } else {
    // Set lock on immediate submitting operations to driver.
    this.submitMap[opData.src] = [];
    this._trySubmit(options);
  }
};

Livedb.prototype._dequeueNextSubmit = function(src) {
  // TODO noansknv Documentation.
  if (this.submitMap[src]) {
    if (this.submitMap[src].length === 0) {
      delete this.submitMap[src];
    } else {
      var options = this.submitMap[src].splice(0, 1);

      var self = this;
      // Callstack could potentially be very large, so lets clean it up.
      setTimeout(function() {
        self._trySubmit.apply(self, options);
      }, 0);
    }
  }
}

Livedb.prototype._trySubmit = function(options) {
  var self = this;
  var opData = options.opData;
  ...
  this._lazyFetch(options.cName, options.docName, function(err, snapshot) {
    if (err) {
      options.callback(err);
      self._dequeueNextSubmit(opData.src);
      return;
    }
    ...
    self.driver.getOps(options.cName, options.docName, from, null, function(err, ops) {
      if (err) {
        options.callback(err);
        self._dequeueNextSubmit(opData.src);
        return;
      ...
      if (options.expectTransform && ops.length === 0) {
        console.warn("ERROR: CORRUPT DATA DETECTED in document " + options.cName + '.' + options.docName);
        console.warn("If you're using redis, delete data for document. "
          + "Please file an issue if you can recreate this state reliably.");
        options.callback('Internal data corruption - cannot submit');
        self._dequeueNextSubmit(opData.src);
        return;
      }
      ...
      for (var i = 0; i < ops.length; i++) {
        ...
        if (opData.src && opData.src === op.src && opData.seq === op.seq) {
          ...
          // & submit again.
          options.callback('Op already submitted');
          self._dequeueNextSubmit(opData.src);
          return;
        }
        ...
        if (opData.v === op.v) {
          options.transformedOps.push(op);
      ...
      if (opData.v == null)
        ...
        options.callback('Invalid opData version');
        self._dequeueNextSubmit(opData.src);
        return;
      }
      ...
      if (options.projection && !projections.isOpDataAllowed(type, options.projection.fields, opData)) {
        options.callback('Operation invalid in projected collection');
        self._dequeueNextSubmit(opData.src);
        return;
      }
      ...
      var dirtyData = self.getDirtyDataPre(options.cName, options.docName, opData, snapshot);
      ...
      var dirtyDataPost = self.getDirtyData(options.cName, options.docName, opData, snapshot);
      ...
      options.submitOptions.dirtyData = dirtyData;
      ...
      self.driver.atomicSubmit(options.cName, options.docName, opData, options.submitOptions, function(err) {
        if (err === 'Transform needed') {
          // TODO noansknv: Update comments!
          ...
          options.expectTransform = true;
          self._trySubmit(options);
        } else if (err) {
          options.callback(err);
          self._dequeueNextSubmit(opData.src);
        } else {
          self._writeSnapshotAfterSubmit(options.cName, options.docName, snapshot, opData, options.submitOptions, function(err) {
            ...
            // self._updateCursors(options.cName, options.docName, type, opData);
            if (self.driver.postSubmit) self.driver.postSubmit(options.cName, options.docName, opData, snapshot);

            if (self.sdc) self.sdc.timing('livedb.submit', Date.now() - options.start);

            options.callback(err, snapshot.v - 1, options.transformedOps, snapshot);
            self._dequeueNextSubmit(opData.src);
          });
        }
      });
