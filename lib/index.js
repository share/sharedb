...
Livedb.prototype.submit = function(cName, docName, opData, options, callback) {
  ...
  // TODO noansknv: What is the better way of passing arguments?
  var args = [
    cName,
    docName,
    opData,
...
Livedb.prototype._dequeueNextSubmit = function(src) {
  ...
  if (this.submitMap[src]) {
    if (this.submitMap[src].length === 0) {
      ...
      var args = this.submitMap[src].splice(0, 1)[0];

      var self = this;
      // Callstack could potentially be very large, so lets clean it up.
      setTimeout(function() {
        self._trySubmit.apply(self, args);
      }, 0);
    }
  }
}

Livedb.prototype._trySubmit = function(cName, docName, opData, options, callback, start, projection, transformedOps, expectTransform) {
  var self = this;
  ...
  this._lazyFetch(cName, docName, function(err, snapshot) {
    if (err) {
      callback(err);
    ...
    self.driver.getOps(cName, docName, from, null, function(err, ops) {
      ...
      if (expectTransform && ops.length === 0) {
        ...
        callback('Internal data corruption - cannot submit');
        self._dequeueNextSubmit(opData.src);
        return;
      }
      ...
      if (opData.v == null)
        ...
        callback('Invalid opData version');
        self._dequeueNextSubmit(opData.src);
        return;
      }
      ...
      if (projection && !projections.isOpDataAllowed(type, projection.fields, opData)) {
        callback('Operation invalid in projected collection');
        self._dequeueNextSubmit(opData.src);
        return;
      }
      ...
      var dirtyData = self.getDirtyDataPre(cName, docName, opData, snapshot);
      ...
      var dirtyDataPost = self.getDirtyData(cName, docName, opData, snapshot);
      ...
      self.driver.atomicSubmit(cName, docName, opData, options, function(err) {
        if (err === 'Transform needed') {
          // TODO noansknv: remove this line?
          ...
          self._trySubmit(cName, docName, opData, options, callback, start, projection, transformedOps, true);
        } else if (err) {
          callback(err);
          self._dequeueNextSubmit(opData.src);
        } else {
...
Livedb.prototype._trySubmitOpToSnapshot = function(err, snapshot) {};
