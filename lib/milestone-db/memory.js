...
MemoryMilestoneDB.prototype.getMilestoneSnapshot = function(collection, id, version, callback) {
  if (!this._isValidVersion(version)) {
    return process.nextTick(callback, new ShareDBError(ERROR_CODE.UNKNOWN_ERROR, 'Invalid version'));
  }
...
MemoryMilestoneDB.prototype.saveMilestoneSnapshot = function(collection, snapshot, callback) {
  ...
  if (!collection) return callback(new ShareDBError(ERROR_CODE.UNKNOWN_ERROR, 'Missing collection'));
  if (!snapshot) return callback(new ShareDBError(ERROR_CODE.UNKNOWN_ERROR, 'Missing snapshot'));
...
MemoryMilestoneDB.prototype.getMilestoneSnapshotAtOrBeforeTime = function(collection, id, timestamp, callback) {
  if (!this._isValidTimestamp(timestamp)) {
    return process.nextTick(callback, new ShareDBError(ERROR_CODE.UNKNOWN_ERROR, 'Invalid timestamp'));
  }
...
MemoryMilestoneDB.prototype._findMilestoneSnapshot = function(collection, id, breakCondition, callback) {
  if (!collection) return process.nextTick(callback, new ShareDBError(ERROR_CODE.UNKNOWN_ERROR, 'Missing collection'));
  if (!id) return process.nextTick(callback, new ShareDBError(ERROR_CODE.UNKNOWN_ERROR, 'Missing ID'));
