...
function StatelessPresence (doc) {
  ...
  // The minimum amount of time to wait before removing processed presence from this.received.
...
StatelessPresence.prototype.submit = function (data, callback) {
  if (data != null) {
    if (!this.doc.type) {
      var doc = this.doc;
  ...
  if (this._setPresence('', data, true) || this.pending || this.doc.presence.inflight) {
    if (!this.pending) {
      this.pending = [];
    }
    if (callback) {
...
StatelessPresence.prototype.transformAllPresence = function (op) {
  ...
  for (var i = 0; i < srcList.length; i++) {
    var src = srcList[i];
    if (this.doc._transformPresence(src, op)) {
      changedSrcList.push(src);
    }
  }
...
StatelessPresence.prototype.pause = function () {
  if (!this) return;

  if (this.inflight) {
    this.pending = this.doc.presence.pending
      ? this.inflight.concat(this.doc.presence.pending)
      : this.inflight;
    this.inflight = null;
    this.inflightSeq = 0;
  } else if (!this.pending && this.doc.presence.current[''] != null) {
    this.pending = [];
  }
  this.received = {};
  this.requestReply = true;
...
StatelessPresence.prototype.cacheOp = function (op) {
  if (!this) return;
  // Remove the old ops.
...
// If there are no pending ops, this.doc method sends the pending presence data, if possible.
StatelessPresence.prototype.flush = function () {
  if (this.doc.subscribed && !this.inflight && this.doc.presence.pending && !this.doc.hasWritePending()) {
    this.inflight = this.doc.presence.pending;
    this.inflightSeq = this.doc.connection.seq;
    this.pending = null;
    this.doc.connection.sendPresence(this.doc, this.current[''], this.doc.presence.requestReply);
    this.requestReply = false;
  }
}

StatelessPresence.prototype.destroy = function () {
  this.received = {};
  this.cachedOps.length = 0;
}

// Reset presence-related properties.
StatelessPresence.prototype.hardRollback = function () {
  var pendingPresence = [];
  if (this.inflight) pendingPresence.push(this.doc.presence.inflight);
  if (this.pending) pendingPresence.push(this.doc.presence.pending);
...
StatelessPresence.prototype.hasPending = function () {
