...
function submitPresence(data, callback) {
  ...
  if (this.doc._setPresence('', data, true) || this.pending || this.inflight) {
    if (!this.pending) {
      this.pending = [];
    }
    if (callback) {
...
function _transformPresence(src, op) {
  ...
  return this.doc._setPresence(src, presenceData);
}
 
function transformAllPresence(op) {
...
function pausePresence() {
  ...
  for (var i = 0; i < srcList.length; i++) {
    var src = srcList[i];
    if (src && this.doc._setPresence(src, null)) {
      changedSrcList.push(src);
    }
  }
  this._emitPresence(changedSrcList, false);
...
function _setPresence(src, data, emit) {
  ...
  if (emit) this.presence._emitPresence([ src ], true);
  return true;
}
