...
function projectEdit(fields, op) {
  ...
  for (var i = 0; i < op.length; i++) {
    ...
    if (path.length === 0) {
      ...
      if (isFieldAllowed(fields, path[0])) {
        result.push(c);
      }
    }
  }
...
function isSnapshotAllowed(fields, snapshot) {
  ...
  for (var k in snapshot.data) {
    if (!isFieldAllowed(fields, k)) return false;
  }
  return true;
}
...
function isEditAllowed(fields, op) {
  for (var i = 0; i < op.length; i++) {
    ...
    if (c.p.length === 0) {
      return false;
    } else if (!isFieldAllowed(fields, c.p[0])) {
      return false;
    }
  }
  return true;
...
function isFieldAllowed(fields, field) {
  return util.hasOwn(fields, field) && !!fields[field];
}
