...
var ot_json0_1 = require('ot-json0');
...
function projectSnapshot(fields, snapshot) {
  ...
  if (snapshot.type && snapshot.type !== ot_json0_1.type.uri) {
    throw new Error(
      ERROR_CODE.ERR_TYPE_CANNOT_BE_PROJECTED,
      'Cannot project snapshots of type ' + snapshot.type,
    );
  }
...
function isOpAllowed(knownType, fields, op) {
  ...
  if ('op' in op) {
    if (knownType && knownType !== ot_json0_1.type.uri) return false;
