...
var apply = exports.apply = function(data, opData) {
  ...
  if (opData.create) { // Create operations
    ...
    // We copy any of the op's metadata into the snapshot's metadata so that we
    // can run queries against deleted documents who are otherwise orphaned.
    // this leaves the mtime (the last modified time of the snapshot before the delete)
    // as well as the ts of the delete op.
    var m = opData.m || {};
    var key;
    for (key in m) {
      if(key && m[key]) {
