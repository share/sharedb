...
function checkKey(key) {
  if (key.charAt(0) === '_' && key !== '_selection') return 'Cannot set reserved value';
}
...
exports.transformPresence = function(type, pOp, opData) {
  if (typeof type === 'string') {
    type = otTypes[type];
    if (!type) return "Type not found";
  }
  ...
  if (pOp.p && pOp.p.length === 2 && pOp.p[1] === '_selection' && opData.op) {
    // Gasp...
...
exports.updatePresence = function(type, p, opData) {
  ...
  for (var id in p.data) {
    var d = p.data[id];
    if (d != null && d._selection != null) {
      if (opData.op) {
        d._selection = type.transformCursor(d._selection, opData.op, id === opData.src);
      } else if (opData.del) {
        delete d._selection;
      }
    }
