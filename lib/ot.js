...
exports.checkOp = function(op) {
  ...
  // FIXME: Confusing use of ! and !== in this if statement
  if (!op.seq !== !op.src) { // jshint ignore:line
    return {code: 4013, message: 'seq but not src'};
  }
...
exports.apply = function(snapshot, op) {
  ...
  if (op.create) {
    ...
    try {
      var data = (type.serialize) ? type.deserialize(create.data) : create.data;
      data = type.create(data);
      snapshot.data = (type.serialize) ? type.serialize(data) : data;
...
function applyOpEdit(snapshot, edit) {
  ...
  try {
    var data = (type.serialize) ? type.deserialize(snapshot.data) : snapshot.data;
    data = type.apply(data, edit);
    snapshot.data = (type.serialize) ? type.serialize(data) : data;
  } catch (err) {
    return err;
  }
