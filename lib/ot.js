...
exports.checkOp = function(op) {
  if (op == null || typeof op !== 'object') {
    return {code: ERROR_CODE.ERR_OT_OP_BADLY_FORMED, message: 'Op must be an object'};
  }

  if (op.create != null) {
    if (typeof op.create !== 'object') {
      return {code: ERROR_CODE.ERR_OT_OP_BADLY_FORMED, message: 'Create data must be an object'};
    }
    var typeName = op.create.type;
    if (typeof typeName !== 'string') {
      return {code: ERROR_CODE.ERR_OT_OP_BADLY_FORMED, message: 'Missing create type'};
    }
    ...
    return {code: ERROR_CODE.ERR_OT_OP_BADLY_FORMED, message: 'Missing op, create, or del'};
  }

  if (op.src != null && typeof op.src !== 'string') {
    return {code: ERROR_CODE.ERR_OT_OP_BADLY_FORMED, message: 'src must be a string'};
  }
  if (op.seq != null && typeof op.seq !== 'number') {
    return {code: ERROR_CODE.ERR_OT_OP_BADLY_FORMED, message: 'seq must be a string'};
  }
  if (
    (op.src == null && op.seq != null) ||
    (op.src != null && op.seq == null)
  ) {
    return {code: ERROR_CODE.ERR_OT_OP_BADLY_FORMED, message: 'Both src and seq must be set together'};
  }

  if (op.m != null && typeof op.m !== 'object') {
    return {code: ERROR_CODE.ERR_OT_OP_BADLY_FORMED, message: 'op.m must be an object or null'};
  }
};
...
exports.apply = function(snapshot, op) {
  if (typeof snapshot !== 'object') {
    return {code: ERROR_CODE.ERR_APPLY_SNAPSHOT_NOT_PROVIDED, message: 'Missing snapshot'};
  }
  if (snapshot.v != null && op.v != null && snapshot.v !== op.v) {
    return {code: ERROR_CODE.ERR_APPLY_OP_VERSION_DOES_NOT_MATCH_SNAPSHOT, message: 'Version mismatch'};
  }

  // Create operation
  if (op.create) {
    if (snapshot.type) return {code: ERROR_CODE.ERR_DOC_ALREADY_CREATED, message: 'Document already exists'};
...
function applyOpEdit(snapshot, edit) {
  if (!snapshot.type) return {code: ERROR_CODE.ERR_DOC_DOES_NOT_EXIST, message: 'Document does not exist'};
...
exports.transform = function(type, op, appliedOp) {
  ...
  if (op.v != null && op.v !== appliedOp.v) {
    return {code: ERROR_CODE.ERR_OP_VERSION_MISMATCH_DURING_TRANSFORM, message: 'Version mismatch'};
  }

  if (appliedOp.del) {
    if (op.create || op.op) {
      return {code: ERROR_CODE.ERR_DOC_WAS_DELETED, message: 'Document was deleted'};
    }
  } else if (
    ...
    return {code: ERROR_CODE.ERR_DOC_ALREADY_CREATED, message: 'Document was created remotely'};
  } else if (appliedOp.op && op.op) {
    // If we reach here, they both have a .op property.
    if (!type) return {code: ERROR_CODE.ERR_DOC_DOES_NOT_EXIST, message: 'Document does not exist'};
