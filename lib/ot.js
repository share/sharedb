...
function checkOp(op) {
  ...
  if ((op.src == null && op.seq != null) || (op.src != null && op.seq == null)) {
    return new ShareDBError(
      ...
      'Both src and seq must be set together',
    );
  }
...
function apply(snapshot, op) {
  ...
  if (snapshot.v != null && op.v != null && snapshot.v !== op.v) {
    return new ShareDBError(
      ERROR_CODE.ERR_APPLY_OP_VERSION_DOES_NOT_MATCH_SNAPSHOT,
      'Version mismatch',
    );
  }
  // Create operation
...
function transform(type, op, appliedOp) {
 ...
 */
function applyOps(snapshot, ops, options) {
  options = options || {};
...
function transformPresence(presence, op, isOwnOp) {
