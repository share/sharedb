...
OpStream.prototype.pushOp = function(collection, op) {
  if (this.backend) {
    var stream = this;
    this.backend._sanitizeOp(this.agent, this.projection, collection, op.d, op, function(err) {
...
OpStream.prototype.pushOps = function(collection, ops) {
  for (var i = 0; i < ops.length; i++) {
    this.pushOp(collection, ops[i]);
  }
};
