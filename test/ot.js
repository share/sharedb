...
describe('ot', function () {
    ...
    describe('transform', function () {
        ...
        it('delete by delete ok', function () {
            var op1 = { del: true, v: 6 };
            expect(ot.transform(type.uri, op1, { del: true, v: 6 })).equal();
            expect(op1).eql({ del: true, v: 7 });
            // And with no version specified should work, too
            var op2 = { del: true };
            expect(ot.transform(type.uri, op2, { del: true, v: 6 })).equal();
            expect(op2).eql({ del: true });
        });
        it('delete by op ok', function () {
            var op1 = { del: true, v: 8 };
            expect(ot.transform(type.uri, op1, { op: [], v: 8 }));
            expect(op1).eql({ del: true, v: 9 });
            // And with no version specified should work, too
            var op2 = { del: true };
            expect(ot.transform(type.uri, op2, { op: [], v: 8 }));
            expect(op2).eql({ del: true });
        });
        it('delete by noop ok', function () {
            var op1 = { del: true, v: 6 };
            expect(ot.transform(null, op1, { v: 6 })).equal();
            expect(op1).eql({ del: true, v: 7 });
            var op2 = { del: true };
            expect(ot.transform(null, op2, { v: 6 })).equal();
            expect(op2).eql({ del: true });
        });
        ...
        it('op by op ok', function () {
            var op11 = { v: 6, op: [{ p: [10], si: 'hi' }] };
            var op21 = { v: 6, op: [{ p: [5], si: 'abcde' }] };
            expect(ot.transform(type.uri, op11, op21)).equal();
            expect(op11).eql({ v: 7, op: [{ p: [15], si: 'hi' }] });
            // No version specified
            var op12 = { op: [{ p: [10], si: 'hi' }] };
            var op22 = { v: 6, op: [{ p: [5], si: 'abcde' }] };
            expect(ot.transform(type.uri, op12, op22)).equal();
            expect(op12).eql({ op: [{ p: [15], si: 'hi' }] });
        });
