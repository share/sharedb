  ...
  describe('client presence (' + typeName + ')', function() {
    ...
    it('sends presence immediately', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc.presence.submit(p(1), errorHandler(done));
    ...
    it.skip('waits for pending ops before processing future presence', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc.version += 2;
          this.doc.presence.requestReply = false;
          this.doc.presence.submit(p(1), function(err) {
    ...
    it('handles presence sent for earlier revisions (own ops, presence.index < op.index)', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc.data = [ 'a' ];
          this.doc.presence.requestReply = false;
          this.doc.presence.submit(p(0), errorHandler(done));
        }.bind(this)
      ], allDone);
    });
    ...
    it('transforms presence against local delete', function(allDone) {
      async.series([
        ...
        this.doc.subscribe.bind(this.doc),
        this.doc2.subscribe.bind(this.doc2),
        this.doc.presence.submit.bind(this.doc, p(0)),
        this.doc2.presence.submit.bind(this.doc2, p(0)),
        setTimeout,
        function(done) {
          this.doc.on('presence', function(srcList, submitted) {
            expect(srcList.sort()).to.eql([ '', this.connection2.id ]);
            expect(submitted).to.equal(false);
            expect(this.doc.presence.current).to.not.have.key('');
            expect(this.doc.presence.current).to.not.have.key(this.connection2.id);
            done();
          }.bind(this));
    ...
    it('transforms presence against local op (presence.index != op.index)', function(allDone) {
      async.series([
        ...
        this.doc2.subscribe.bind(this.doc2),
        this.doc.presence.submit.bind(this.doc, p(0)),
        this.doc2.presence.submit.bind(this.doc2, p(2)),
        setTimeout,
        function(done) {
          this.doc.on('presence', function(srcList, submitted) {
            expect(srcList).to.eql([ this.connection2.id ]);
            expect(submitted).to.equal(false);
            expect(this.doc.presence.current['']).to.eql(p(0));
            expect(this.doc.presence.current[this.connection2.id]).to.eql(p(3));
            done();
          }.bind(this));
          this.doc.submitOp({ index: 1, value: 'b' }, errorHandler(done));
        }.bind(this)
    ...
    it('transforms presence against local op (presence.index == op.index)', function(allDone) {
      async.series([
        ...
        this.doc.subscribe.bind(this.doc),
        this.doc2.subscribe.bind(this.doc2),
        this.doc.presence.submit.bind(this.doc, p(1)),
        this.doc2.presence.submit.bind(this.doc2, p(1)),
        setTimeout,
        function(done) {
          this.doc.on('presence', function(srcList, submitted) {
            expect(srcList).to.eql([ '' ]);
            expect(submitted).to.equal(false);
            expect(this.doc.presence.current['']).to.eql(p(2));
            expect(this.doc.presence.current[this.connection2.id]).to.eql(p(1));
            done();
          }.bind(this));
          this.doc.submitOp({ index: 1, value: 'b' }, errorHandler(done));
        }.bind(this)
      ...
      async.series([
        ...
        function(done) {
          this.doc.on('presence', function(srcList, submitted) {
            expect(srcList).to.eql([ this.connection2.id ]);
            expect(submitted).to.equal(false);
            expect(this.doc.presence.current['']).to.eql(p(1));
            expect(this.doc.presence.current[this.connection2.id]).to.eql(p(2));
            done();
          }.bind(this));
          this.doc2.submitOp({ index: 1, value: 'b' }, errorHandler(done));
        }.bind(this)
    ...
    it('requests reply presence when sending presence for the first time', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc2.presence.submit(p(1), errorHandler(done));
        }.bind(this)
      ], allDone);
    });
    ...
    it('fails to submit presence for uncreated document: callback(err)', function(allDone) {
      async.series([
        this.doc.subscribe.bind(this.doc),
        function(done) {
          this.doc.presence.submit(p(0), function(err) {
    ...
    it('submits null presence', function(allDone) {
      async.series([
        this.doc.subscribe.bind(this.doc),
        this.doc.presence.submit.bind(this.doc, null)
      ], allDone);
    });
    ...
    it('sends presence once, if submitted multiple times synchronously', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc.presence.submit(p(0), errorHandler(done));
          this.doc.presence.submit(p(1), errorHandler(done));
          this.doc.presence.submit(p(2), errorHandler(done));
        }.bind(this)
      ], allDone);
    });
        ...
        function(done) {
          ...
          setTimeout(function() {
            this.doc.subscribe(function(err) {
              if (err) return done(err);
              expect(this.doc2.presence.current).to.eql({});
            }.bind(this));
          }.bind(this));
        }.bind(this)
      ], allDone);
    ...
    it('hasPending is true, if there is pending presence', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          expect(!!this.doc.presence.pending).to.equal(true);
          expect(!!this.doc.presence.inflight).to.equal(false);
          this.doc.whenNothingPending(done);
        }.bind(this),
        function(done) {
          expect(this.doc.hasPending()).to.equal(false);
          expect(!!this.doc.presence.pending).to.equal(false);
          expect(!!this.doc.presence.inflight).to.equal(false);
          done();
        }.bind(this)
      ], allDone);
      ...
      async.series([
        ...
        function(done) {
          ...
          this.doc2.on('presence', function(srcList, submitted) {
            ...
            // The call to `presence.submit` does not fire the event because presence is already null.
            expect(submitted).to.equal(false);
            expect(this.doc2.presence.current).to.not.have.key(this.connection.id);
            done();
          }.bind(this));
      ...
      async.series([
        ...
        function(done) {
          ...
          this.doc2.on('presence', function(srcList, submitted) {
            ...
            expect(submitted).to.equal(true);
            expect(this.doc2.presence.current).to.not.have.key(connectionId);
            expect(this.doc2.presence.current['']).to.eql(p(1));
            done();
          }.bind(this));
          this.connection.close();
    ...
    it('pauses inflight and pending presence on disconnect', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc.presence.submit(p(0), callback);
          process.nextTick(function() {
            this.doc.presence.submit(p(1), callback);
            this.connection.close();
            process.nextTick(function() {
              this.backend.connect(this.connection);
            }.bind(this));
    ...
    it('emits the same presence only if comparePresence is not implemented (local presence)', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc.presence.submit(p(1), typeName === 'wrapped-presence-no-compare' ? errorHandler(done) : done);
        }.bind(this)
      ], allDone);
    });
    ...
    it('clears presence on hard rollback and emits an error', function(allDone) {
      async.series([
        ...
        this.doc.presence.submit.bind(this.doc, p(1)), // presence.inflight
        process.nextTick, // wait for "presence" event
        this.doc.presence.submit.bind(this.doc, p(2)), // presence.pending
        process.nextTick, // wait for "presence" event
        function(done) {
    ...
    it('clears presence on hard rollback and executes all callbacks', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc.presence.submit(p(1), callback); // presence.inflight
          process.nextTick(function() { // wait for presence event
            this.doc.presence.submit(p(2), callback); // presence.pending
            process.nextTick(function() { // wait for presence event
              this.doc.on('presence', function(srcList, submitted) {
                expect(presenceEmitted).to.equal(false);
                presenceEmitted = true;
      ...
      return function(allDone) {
        ...
        async.series([
          ...
          function(done) {
            ...
            this.doc2.presence.submit(p(0), done);
          }.bind(this),
          setTimeout,
          ...
          function(done) {
            ...
            expect(this.doc.presence.current[this.connection2.id]).to.eql(p(0));
          ...
          function(done) {
            expect(this.doc.presence.current[this.connection2.id]).to.eql(expireCache ? p(1) : p(0));
            process.nextTick(done);
          }.bind(this)
        ], allDone);
      };
