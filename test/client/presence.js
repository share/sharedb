  ...
  describe('client presence (' + typeName + ')', function() {
    ...
    it('transforms presence against local delete', function(allDone) {
      async.series([
        ...
        async.nextTick, // wait for the doc2 presence message to reach doc
        function(done) {
    ...
    it('clears presence on hard rollback and emits an error', function(allDone) {
      async.series([
        ...
        function(done) {
          ...
          this.doc.submitPresence(p(1)); // inflightPresence
          process.nextTick(function() {
            this.doc.submitPresence(p(2)); // pendingPresence
            ...
            // send an invalid op
            this.doc._submit({ index: 3, value: 'b' }, true);
          }.bind(this));
        }.bind(this)
      ], allDone);
    });
    ...
    function testReceivedMessageExpiry(expireCache, reduceSequence) {
      return function(allDone) {
        ...
        async.series([
          ...
          async.nextTick, // wait for presence to reach this.doc
          this.doc2.submitOp.bind(this.doc2, { index: 1, value: 'b' }), // forces processing of all received presence
          async.nextTick, // wait for op to reach this.doc
          function(done) {
