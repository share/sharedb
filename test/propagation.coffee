...
createInProcessDriver = require '../lib/inprocessdriver'
...
drivers = [{
  ...
  name: 'inprocess'
  create: createInProcessDriver
  setup: ->
  teardown: () ->
    @driver.destroy()
}]

for driver in drivers then do (driver) -> describe "driver:#{driver.name}", ->
  describe 'operation propagation granularity', ->
    beforeEach driver.setup

    describe 'Propagates changes to appropriate channels', ->
      beforeEach -> setup.call(this, new Memory(), driver.create, 'A')
    ...
    describe 'Propagation throttling', () ->
      beforeEach -> setup.call(this, new Memory(), driver.create)
