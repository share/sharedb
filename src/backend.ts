...
import Connection = require('./client/connection');
import emitter = require('./emitter');
import MemoryDB = require('./db/memory');
import NoOpMilestoneDB = require('./milestone-db/no-op');
import MemoryPubSub = require('./pubsub/memory');
import ot = require('./ot');
import projections = require('./projections');
import QueryEmitter = require('./query-emitter');
import ShareDBError = require('./error');
import Snapshot = require('./snapshot');
import StreamSocket = require('./stream-socket');
import SubmitRequest = require('./submit-request');
import ReadSnapshotsRequest = require('./read-snapshots-request');
...
class Backend {
  ...
  maxSubmitRetries;
  presenceEnabled;
  doNotForwardSendPresenceErrorsToClient;
  doNotCommitNoOps;
  ...
  connect(connection, req, callback) {
   ...
   */
  submit(agent, index, id, op, options, originalCallback) {
    var backend = this;
  ...
  sanitizeOp(agent, index, id, op, callback) {
  ...
  _sanitizeOp(agent, projection, collection, id, op, callback) {
    if (projection) {
      try {
  ...
  _sanitizeOps(agent, projection, collection, id, ops, callback) {
    var backend = this;
  ...
  _sanitizeOpsBulk(agent, projection, collection, opsMap, callback) {
    var backend = this;
  ...
  _sanitizeSnapshots(agent, projection, collection, snapshots, snapshotType, callback) {
    if (projection) {
      try {
  ...
  _getSnapshotProjection(db, projection) {
    return (db.projectsSnapshots) ? null : projection;
  }

  _getSnapshotsFromMap(ids, snapshotMap) {
  ...
  _getSanitizedOps(agent, projection, collection, id, from, to, opsOptions, callback) {
    var backend = this;
  ...
  _getSanitizedOpsBulk(agent, projection, collection, fromMap, toMap, opsOptions, callback) {
    var backend = this;
    if (!opsOptions) opsOptions = {};
    if (agent) opsOptions.agentCustom = agent.custom;
   ...
   */
  getOps(agent, index, id, from, to, options, callback) {
  ...
  getOpsBulk(agent, index, fromMap, toMap, options, callback) {
    if (typeof options === 'function') {
      callback = options;
      options = null;
    }
  ...
  fetch(agent, index, id, options, callback) {
    if (typeof options === 'function') {
      callback = options;
      options = null;
    }
   ...
   */
  fetchBulk(agent, index, ids, options, callback) {
    if (typeof options === 'function') {
      callback = options;
      options = null;
    }
  ...
  subscribe(agent, index, id, version, options, callback) {
    if (typeof options === 'function') {
      callback = options;
      options = null;
    }
   ...
   * ) => void} callback
   */
  subscribeBulk(agent, index, versions, callback) {
    var start = Date.now();
    var projection = this.projections[index];
    var collection = (projection) ? projection.target : index;
    var backend = this;
   ...
   *  liveness
   */
  querySubscribe(agent, index, query, options, callback) {
    var start = Date.now();
    var backend = this;
    backend._triggerQuery(agent, index, query, options, function(err, request) {
      if (err) return callback(err);
  ...
  _triggerQuery(agent, index, query, options, callback) {
    var projection = this.projections[index];
    var collection = (projection) ? projection.target : index;
    var fields = projection && projection.fields;
    var request = {
  ...
  getCollectionChannel(collection) {
    return collection;
  }

  getDocChannel(collection, id) {
    return collection + '.' + id;
  }

  getChannels(collection, id) {
    return [
  ...
  fetchSnapshot(agent, index, id, version, callback) {
    var start = Date.now();
    var backend = this;
    var projection = this.projections[index];
    var collection = projection ? projection.target : index;
  ...
  _fetchSnapshot(collection, id, version, callback) {
    var db = this.db;
    var backend = this;
  ...
  fetchSnapshotByTimestamp(agent, index, id, timestamp, callback) {
    var start = Date.now();
    var backend = this;
    var projection = this.projections[index];
    var collection = projection ? projection.target : index;
  ...
  _fetchSnapshotByTimestamp(collection, id, timestamp, callback) {
    var db = this.db;
  ...
  _buildSnapshotFromOps(id, startingSnapshot, ops, callback) {
  ...
  transformPresenceToLatestVersion(agent, presence, callback) {
