...
import Presence = require('./presence/presence');
import DocPresence = require('./presence/doc-presence');
import SnapshotVersionRequest = require('./snapshot-request/snapshot-version-request');
import SnapshotTimestampRequest = require('./snapshot-request/snapshot-timestamp-request');
...
import DocPresenceEmitter = require('./presence/doc-presence-emitter');
...
class Connection {
  ...
  _handleBulkMessage(err, message, method) {
    if (message.data) {
  ...
  _sendBulk(action, collection, values) {
    if (!values) return;
    var ids = [];
  ...
  _sendActions(action, doc, version) {
  ...
  _createQuery(action, collection, q, options, callback) {
   ...
   */
  createFetchQuery(collection, q, options, callback) {
   ...
   * where results is a list of Doc objects.
   */
  createSubscribeQuery(collection, q, options, callback) {
   ...
   *
   */
  fetchSnapshot(collection, id, version, callback) {
   ...
   *
   */
  fetchSnapshotByTimestamp(collection, id, timestamp, callback) {
  ...
  _handleSnapshotFetch(error, message) {
  ...
  _handleHandshake(error, message) {
    if (error) return this.emit('error', error);
    this._initialize(message);
  }

  _handlePingPong(error) {
    if (error) return this.emit('error', error);
    this.emit('pong');
  }
  ...
  _sendPresenceAction(action, seq, presence) {
