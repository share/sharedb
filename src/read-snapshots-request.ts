...
class ReadSnapshotsRequest {
  ...
  constructor(collection, snapshots, snapshotType) {
   ...
   */
  rejectSnapshotRead(snapshot, error) {
    if (!this._idToError) {
   ...
   */
  rejectSnapshotReadSilent(snapshot, errorMessage) {
    this.rejectSnapshotRead(snapshot, this.silentRejectionError(errorMessage));
  }

  silentRejectionError(errorMessage) {
