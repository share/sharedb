...
// ignored by the connection.
    ...
    public _onConnectionStateChanged() {
    ...
    public _handleFetch(err, data, extra) {
        // Once a fetch query gets its data, it is destroyed.
        this.connection._destroyQuery(this);
        this._handleResponse(err, data, extra);
    };

    public _handleSubscribe(err, data, extra) {
        this._handleResponse(err, data, extra);
    };

    public _handleResponse(err, data, extra) {
    ...
    public _ingestSnapshots(snapshots, finish?) {
        var results:any = [];
    ...
    public _finishResponse(err, callback) {
        this.emit('ready');
        this.ready = true;
        if (err) {
            this.connection._destroyQuery(this);
