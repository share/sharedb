...
var Agent = /** @class */ (function () {
    ...
    Agent.prototype._subscribeToStream = function (collection, id, stream) {
        ...
        stream.on('data', function (data) {
            ...
            if (agent._isOwnOp(collection, data))
                return;
    ...
    Agent.prototype._subscribeToQuery = function (emitter, queryId, collection, query) {
        ...
        emitter.onOp = function (op) {
            var id = op.d;
            if (agent._isOwnOp(collection, op))
                return;
    ...
    Agent.prototype._open = function () {
        ...
        if (!this.stream.isServer)
            this.backend.remoteAgentsCount++;
        var agent = this;
        ...
        this.stream.on('end', function () {
            ...
            if (!agent.stream.isServer)
                agent.backend.remoteAgentsCount--;
            agent._cleanup();
        });
    };
    ;
    ...
    Agent.prototype._checkRequest = function (request) {
        if (request.a === 'qf' || request.a === 'qs' || request.a === 'qu') {
            ...
            if (request.c != null && typeof request.c !== 'string')
                return 'Invalid collection';
            if (typeof request.b !== 'object')
                return 'Invalid bulk subscribe data';
        }
    };
    ;
    ...
    Agent.prototype._handleMessage = function (request, callback) {
        try {
            ...
            if (errMessage)
                return callback({ code: 4000, message: errMessage });
            switch (request.a) {
                case 'qf':
                ...
                case 'op':
                    ...
                    if (!op)
                        return callback({ code: 4000, message: 'Invalid op message' });
    ...
    ;
    Agent.getQueryOptions = function (request) {
    ...
    ;
    Agent.getResultsData = function (results) {
        var items = [];
        for (var i = 0; i < results.length; i++) {
            var result = results[i];
    ...
    Agent.getMapData = function (snapshotMap) {
        var data = {};
    ...
    Agent.getSnapshotData = function (snapshot) {
        var data = {
            v: snapshot.v,
            data: snapshot.data
        };
