...
exports.checkOp = function (op) {
    ...
    if (op.create != null) {
        ...
        if (op.del !== true)
            return { code: 4009, message: 'del value must be true' };
    }
    else if (op.op == null) {
...
exports.apply = function (snapshot, op) {
    ...
    if (op.create) {
        if (snapshot.type)
            return { code: 4016, message: 'Document already exists' };
...
function applyOpEdit(snapshot, edit) {
    if (!snapshot.type)
        return { code: 4015, message: 'Document does not exist' };
    if (typeof edit !== 'object')
        return { code: 5004, message: 'Missing op' };
...
exports.transform = function (type, op, appliedOp) {
    ...
    if (appliedOp.del) {
    ...
    else if ((appliedOp.create && (op.op || op.create || op.del)) ||
        ...
        if (!type)
            return { code: 5005, message: 'Document does not exist' };
